...
const StarField: React.FC = () => {
  ...
  const positions = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (Math.abs(x) < 20 && y > -5 && y < 20) { if (x < 0) x -= 20; else x += 20; }
  ...
  return (
    <points ref={meshRef}>
      ...
      <pointsMaterial size={0.8} color="#ffffff" transparent opacity={0.6} sizeAttenuation />
    </points>
  );
};
        ...
        <group position={[0, 0.02, 0]}>
            ...
            {separators.map((x, i) => (
                <mesh key={`sep-${i}`} position={[x, 0, -20]} rotation={[-Math.PI / 2, 0, 0]}>
                    <planeGeometry args={[0.05, 200]} /><meshBasicMaterial color="#00ffff" transparent opacity={0.4} />
                </mesh>
            ))}
        </group>
    );
...
const RetroSun: React.FC = () => {
    return (<group position={[0, 10, -200]}><mesh><sphereGeometry args={[12, 16, 16]} /><meshBasicMaterial color="#ff0080" /></mesh></group>);
};
...
const MovingGrid: React.FC = () => {
    ...
    useFrame((state, delta) => {
        if (meshRef.current) {
             ...
             meshRef.current.position.z = -100 + (offsetRef.current % 10);
        }
    });
    return (<mesh ref={meshRef} rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.2, -100]}><planeGeometry args={[300, 400, 20, 20]} /><meshBasicMaterial color="#8800ff" wireframe transparent opacity={0.15} /></mesh>);
};
