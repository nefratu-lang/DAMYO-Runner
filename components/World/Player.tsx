...
export const Player: React.FC = () => {
  ...
  // --- Controls (Keyboard & Touch & On-Screen) ---
  const triggerJump = () => {
  ...
  const moveLeft = () => setLane(l => Math.max(l - 1, 0));
  const moveRight = () => setLane(l => Math.min(l + 1, laneCount - 1));
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      else if (e.key === 'ArrowUp' || e.key === 'w' || e.key === ' ' || e.key === 'Enter') triggerJump();
    };

    // Custom Event for On-Screen Buttons
    const handleGameControl = (e: Event) => {
        if (status !== GameStatus.PLAYING) return;
        const detail = (e as CustomEvent).detail;
        if (detail === 'LEFT') moveLeft();
        if (detail === 'RIGHT') moveRight();
        if (detail === 'JUMP') triggerJump();
    };
  ...
  // Keep touch for those who still want it
  useEffect(() => {
